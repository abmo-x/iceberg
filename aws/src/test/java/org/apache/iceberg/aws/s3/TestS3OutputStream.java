...
public class TestS3OutputStream {
  ...
  public void testAbortAfterFailedPartUpload() {
    doThrow(new RuntimeException("mock upload part failure")).when(s3mock).uploadPart((UploadPartRequest) any(), (RequestBody) any());
  ...
  public void testAbortMultipart() {
    doThrow(new RuntimeException("mock upload part failure")).when(s3mock).completeMultipartUpload((CompleteMultipartUploadRequest) any());
